...
const MasterCommissions = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [balanceData, transactionsData, walletBalanceData] = await Promise.all([commissionService.getBalance(), commissionService.getTransactions(filter === "all" ? undefined : filter), walletService.getBalance()]);
            ...
            {transactions.map((transaction) => (
              ...
              >
                ...
                <div style={{ textAlign: "right", display: "flex", flexDirection: "column", gap: "12px", alignItems: "flex-end" }}>
                  <div style={{ fontSize: "1.5rem", fontWeight: "700", color: "#2d3748" }}>{formatPrice(transaction.commission_amount)}</div>
                  <div
                    style={{
                  ...
                  {transaction.status === 'pending' && (
                    <button
                      onClick={() => handlePayFromWallet(transaction.id, transaction.commission_amount)}
                      disabled={payingId === transaction.id}
                      style={{
                        padding: "10px 20px",
                        background: payingId === transaction.id ? "#cbd5e0" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
                        color: "white",
                        border: "none",
                        borderRadius: "8px",
                        fontSize: "0.95rem",
                        fontWeight: "600",
                        cursor: payingId === transaction.id ? "not-allowed" : "pointer",
                        display: "flex",
                        alignItems: "center",
                        gap: "8px",
                        transition: "all 0.2s",
